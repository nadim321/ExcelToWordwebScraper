...
public class Asdnews {

    public static void main(String[] args) {
        // Configuration
        String excelPath = "input_urls.xlsx"; // Excel with URLs
        String wordOutputPath = "batch_1_defense_gov.docx";
...
//        String targetUrl = "https://www.asdnews.com/news/defense/2023/10/11/gaasi-advances-aerial-recovery-suas-ale";
        ...
        try {
            ...
            Elements paragraphs = doc.select("p");
            for (Element p : paragraphs) {
                if (p.getElementsByAttributeValue("class", "mb-1").size() > 0){
                    break;
                }
                String text = p.text().trim();
                if (text.contains("GA-ASI's MQ-1C Gray Eagle ER Featured at US Army's PC-C5")){
                    break;
                }
                if (!text.isEmpty()) {
                    content.append(text).append("\n\n");
